...
class DocScanner(object):
    ...
    def get_contour(self, rescaled_image, output_path=None):
        ...
        # "edged" type=numpy.array; shape=(500, 538) là hình ảnh edge
    ...
    def scan(self, image, vis_img_path=None, txt_path=None, binary=False):
        ...
        orig = image.copy()
        # giả sử Input_Image.shape = (1724, 1858, 3)
        # --> "rescaled_image" = (500, 538, 3)
        rescaled_image = imutils.resize(image, height = int(RESCALED_HEIGHT))

        # get the contour of the document
        # tọa độ 4 góc của hình chữ nhật
        screenCnt = self.get_contour(rescaled_image)

        # apply the perspective transformation
        warped, rect = transform.four_point_transform(orig, screenCnt * ratio)
        ...
        if vis_img_path is not None:
            ...
            cv2.imwrite(vis_img_path, warped)
            if txt_path is not None:
                with open(txt_path, "w") as file:
                    file.write("[" + "], [".join([", ".join([str(ele) for ele in item]) for item in rect]) + "]")
        else:
            return warped, rect
...
if __name__ == "__main__":
    ...
    if im_file_path:
        ...
        # giả sử có tên file là image.png thì file_name="image"
        file_name = im_file_path[:-4].split("/")[-1]
        scanner.scan(image, os.path.join(args.output, file_name + "_scanned.jpg"))
    ...
    # Scan all valid images in directory specified by command line argument --images <IMAGE_DIR>
        ...
        for im in im_files:
            ...
            scanner.scan(image, output_path)