...
class BatchCollateFn(object):
    ...
    def __call__(self, batch_list: List[Document]):
        ...
        # org_shape_list, (B, num_boxes, 2)
        if not self.training:
            org_shape_list = [torch.tensor([doc.org_width, doc.org_height], dtype=torch.int32) for doc in batch_list]
            org_shape_tensor = torch.stack(org_shape_list, dim=0)
